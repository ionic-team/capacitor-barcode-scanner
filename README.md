...
Note: iOS supports all formats but `UPC_EAN_EXTENSION` - using it in `hint` will default to scanning any format. Also, Apple Vision does not distinguish between `UPC_A` and `UPC_E`, so specifying one of these in `hint` will allow to scan both.
